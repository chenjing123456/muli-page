...
function getEntNameMap (orderNo,token){
    $.ajax({
        url: "/api/entDetail/postEntNameMap?entName=" + orderNo + '&token=' + token,
        type: 'GET',
        timeout: 600000,
...
function nodeStyle(node) {
    ...
    if (isCompany(node.id)) {
        if (node.data.colorType[0].split('-')[2] === 'target') {
            ...
            node.display = 'image';
            node.image = "/static/plugins/images/gd.png";
        } else if (node.data.colorType[0].split('-')[2] == 'dwtz') {
        ...
        if (node.data.colorType[0].split('-')[2] == 'gd') {
            ...
            node.display = 'image';
            node.image =  "/static/plugins/images/rz.png";
        } else if (node.data.colorType[0].split('-')[2] == 'djg') {
            node.display = 'image';
            node.image =  "/static/plugins/images/djg.png";
        } else {
            node.fillColor = "#0AADFF";
        }
    }
